...
// An STL-compatible iterator that forwards calls to some underlying
// implementation. Often, we want to pass iterators by value - in fact, the STL
// requires that iterators be copyable. But copying polymorphic objects is
// dangerous because of slicing. Using this class lets us sidestep this issue
// and provide a copyable class that emulates polymorphic behavior.
class TableIterator {
 public:
  ...
  TableIterator(const TableIterator& other) : impl_{other.impl_->Clone()} {}
